...
public final class ReconciliationOverlay {
    ...
    private static void attachSettings(Activity a){
        ...
        for(int i=0;i<parent.getChildCount();i++)if(parent.getChildAt(i) instanceof Button&&((Button)parent.getChildAt(i)).getText().toString().contains("Unificar movimientos repetidos"))return;
        Button b=new Button(a);b.setText("🔎 Unificar movimientos repetidos");b.setAllCaps(false);b.setTextColor(ThemePrefs.isLight(a)?Color.rgb(31,38,44):Color.rgb(242,245,247));GradientDrawable bg=new GradientDrawable();bg.setColor(ThemePrefs.isLight(a)?Color.WHITE:Color.rgb(22,32,41));bg.setCornerRadius(dp(a,14));b.setBackground(bg);
    ...
    private static void attachHelp(Activity a){
        ...
        help.setOnClickListener(v->new AlertDialog.Builder(a).setTitle("Ayuda · ControlGastos").setItems(new String[]{"Ayuda general","Análisis automático del mes","Unificación de movimientos repetidos"},(d,w)->{if(w==0)invokeGeneralHelp(a);else if(w==1)showAnalysisHelp(a);else showReconciliationHelp(a);}).setNegativeButton("Cerrar",null).show());
    }
