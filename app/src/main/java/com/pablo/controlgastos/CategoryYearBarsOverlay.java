...
public final class CategoryYearBarsOverlay {
    ...
    private static void showInPage(Activity a){
        ...
        Runnable refresh=()->{
            ...
            chart.setData(cat,values,CategoryPrefs.color(a,cat),end);
    ...
    static class CategoryBarsView extends View{
        private final Paint p=new Paint(Paint.ANTI_ALIAS_FLAG);private final boolean light;private ArrayList<MonthValue> data=new ArrayList<>();private int barColor=Color.rgb(255,193,7);private String category="";private Calendar endMonth;private final RectF[] hit=new RectF[12];
        CategoryBarsView(Context c,boolean l){super(c);light=l;setLayerType(View.LAYER_TYPE_SOFTWARE,null);}
        void setData(String cat,ArrayList<MonthValue> d,int color,Calendar end){category=cat;data=d;barColor=color;endMonth=(Calendar)end.clone();invalidate();}
        ...
        @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_UP)return true;float x=e.getX(),y=e.getY();for(int i=0;i<hit.length&&i<data.size();i++){RectF r=hit[i];if(r!=null&&x>=r.left-dp(getContext(),6)&&x<=r.right+dp(getContext(),6)&&y>=r.top-dp(getContext(),20)&&y<=r.bottom+dp(getContext(),10)){MonthValue m=data.get(i);showMonthMovements(i,m);break;}}return true;}
        private void showMonthMovements(int index,MonthValue m){if(endMonth==null)return;Calendar from=(Calendar)endMonth.clone();from.set(Calendar.DAY_OF_MONTH,1);from.set(Calendar.HOUR_OF_DAY,0);from.set(Calendar.MINUTE,0);from.set(Calendar.SECOND,0);from.set(Calendar.MILLISECOND,0);from.add(Calendar.MONTH,index-11);Calendar to=(Calendar)from.clone();to.add(Calendar.MONTH,1);ExpenseDb db=new ExpenseDb(getContext());LinearLayout box=new LinearLayout(getContext());box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(getContext(),16),dp(getContext(),4),dp(getContext(),16),dp(getContext(),8));ScrollView sc=new ScrollView(getContext());sc.addView(box);Cursor q=db.getReadableDatabase().rawQuery("SELECT description,amount,ts,source FROM tx WHERE type='GASTO' AND currency='UYU' AND ts>=? AND ts<? ORDER BY ts DESC,id DESC",new String[]{String.valueOf(from.getTimeInMillis()),String.valueOf(to.getTimeInMillis())});SimpleDateFormat df=new SimpleDateFormat("dd/MM/yyyy",new Locale("es","UY"));double total=0;int count=0;while(q.moveToNext()){String desc=q.getString(0),rawCat="";double amount=q.getDouble(1);long ts=q.getLong(2);Cursor cq=db.getReadableDatabase().rawQuery("SELECT category FROM tx WHERE description=? AND ts=? LIMIT 1",new String[]{desc==null?"":desc,String.valueOf(ts)});if(cq.moveToFirst())rawCat=cq.getString(0);cq.close();String key=categoryKey(getContext(),rawCat,desc);if(!category.equalsIgnoreCase(key))continue;total+=amount;count++;LinearLayout row=new LinearLayout(getContext());row.setOrientation(LinearLayout.VERTICAL);row.setPadding(dp(getContext(),10),dp(getContext(),8),dp(getContext(),10),dp(getContext(),8));GradientDrawable g=new GradientDrawable();g.setColor(light?Color.WHITE:Color.rgb(22,32,41));g.setCornerRadius(dp(getContext(),10));row.setBackground(g);row.addView(label(getContext(),(desc==null||desc.trim().isEmpty()?"Sin descripción":desc)+"     $ "+MONEY.format(amount),15,light?Color.rgb(31,38,44):Color.rgb(242,245,247)));row.addView(label(getContext(),df.format(new Date(ts)),12,light?Color.rgb(95,105,112):Color.rgb(158,169,178)));LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(-1,-2);rp.setMargins(0,dp(getContext(),3),0,dp(getContext(),3));box.addView(row,rp);}q.close();box.addView(label(getContext(),count+" movimientos · Total $ "+MONEY.format(total),14,light?Color.rgb(95,105,112):Color.rgb(158,169,178)));new AlertDialog.Builder(getContext()).setTitle(category+" · "+m.month).setView(sc).setPositiveButton("Cerrar",null).show();}
